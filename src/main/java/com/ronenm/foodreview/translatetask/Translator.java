            ...
            for (int i = 0; i < numOfLines; i = i + parallelTranslation) {
                ...
                myPool.submit(() ->
                        list.parallelStream().forEach(line ->
                                {
                                    try {
                                        String[] arrText = line.split(",", -1);
                                        String text = arrText[9];
                                        String translatedText = callTranslateService("https://api.google.com/translate", "en", "fr", text);
                                        arrText[9] = translatedText;
                                        System.out.println(String.join(",", arrText));
